...
cd "$BASE/Публикация"
# цифры питания и сна — из дневника и файла сна, чтобы не переносить руками
node "$BASE/обновить_прогресс.js"
# проверка до копирования: кривое поле роняет render() и страница человека белеет
node "$BASE/Публикация/проверить_книжку.js" "$BASE/Медкнижка.html"
